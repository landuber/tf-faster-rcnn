  ...
  def train_model(self, sess, max_iters):
    ...
    if lsf == 0:
	  ...
	  if v.name == 'vgg_16/fc8/biases:0' or v.name == 'vgg_16/fc8/weights:0':
	    continue
	  if v.name.endswith('conv1/conv1_1/biases:0') or v.name.endswith('conv1/conv1_1/weights:0'):
	    continue
    ...
    while iter < max_iters + 1:
      ...
      if rpn_loss_box > 5:
          print('Problem id: %d  iter: %d / %d, total loss: %.6f\n >>> rpn_loss_cls: %.6f\n '
              '>>> rpn_loss_box: %.6f\n >>> loss_cls: %.6f\n >>> loss_corner: %.6f\n >>> lr: %f' % \
              (blob['roidb_id'], iter, max_iters, total_loss, rpn_loss_cls, rpn_loss_box, loss_cls, loss_corner, lr.eval()))
          print('speed: {:.3f}s / iter'.format(timer.average_time))
